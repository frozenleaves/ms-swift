...
from swift.utils import (HfConfigFactory, Processor, deep_getattr, get_dist_setting, get_env_args, get_logger, is_mp,
                         is_torch_supa_available, to_device)
...
def get_default_device_map():
    ...
    if is_torch_npu_available():
        ...
        return 'auto' if is_mp() else f'supa:{local_rank}'
...
def get_default_torch_dtype(torch_dtype: Optional[torch.dtype]):
    ...
    try:
        is_bf16_available = is_torch_bf16_gpu_available() or (
            is_torch_npu_available() and torch.npu.is_bf16_supported()) or (is_torch_supa_available()
                                                                            and torch.supa.is_bf16_supported())
    except Exception:  # noqa
        is_bf16_available = False

    if is_torch_cuda_available() or is_torch_npu_available() or is_torch_supa_available():
